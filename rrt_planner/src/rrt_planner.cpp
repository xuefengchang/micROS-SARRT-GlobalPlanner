...
bool* OGM2;
RRT myRRT;//RRT树结构
ofstream irrtlog("/home/xuefengchang/irrtlog.txt");
...
/*
void rrt_planner::getGridVal(vector<vector<float> >& gridVal, int startRowID, int startColID,int goalRowID,int goalColID)  //广度优先搜索
{
    int subwidth=width/mapReduceSize,subheight=height/mapReduceSize;
    int i , j , m , n , s;
    float gridMap[subheight][subwidth];
    bool unvisited[subheight][subwidth];
    ...
    Cell goalCell;
    goalCell.posX=goalIdx[0];
    goalCell.posY=goalIdx[1];
    gridVal[goalIdx[0]][goalIdx[1]] =0;
    visited.push(goalCell);
    ...
    while(!visited.empty())
    {
        ...
        vector<Cell> currentFreeNeighborCells;
        
        Cell neighborCell;
        vector <Cell>  freeNeighborCells;

        neighborCell.posX=currentCell.posX-1;
        neighborCell.posY=currentCell.posY;
        if(gridMap[neighborCell.posX][neighborCell.posY]&&unvisited[neighborCell.posX][neighborCell.posY])
            currentFreeNeighborCells.push_back(neighborCell);

        neighborCell.posX=currentCell.posX;
        neighborCell.posY=currentCell.posY-1;
        if(gridMap[neighborCell.posX][neighborCell.posY]&&unvisited[neighborCell.posX][neighborCell.posY])
            currentFreeNeighborCells.push_back(neighborCell);
        ...
        //printf("%d\n", currentFreeNeighborCells.size());
        for(i=0;i<currentFreeNeighborCells.size();i++)
        {
            gridVal[(currentFreeNeighborCells[i]).posX][(currentFreeNeighborCells[i]).posY]=gridVal[currentCell.posX][currentCell.posY]+1;
            visited.push(currentFreeNeighborCells[i]);
            unvisited[(currentFreeNeighborCells[i]).posX][(currentFreeNeighborCells[i]).posY]=false;
        }
    }
}
*/
...
std::vector<geometry_msgs::PoseStamped> rrt_planner::middleoptimizationLayer( std::vector<geometry_msgs::PoseStamped> temp_transformed_plan)
{
    ...
    if((transformed_plan.size())>=3)  //当局部路径大于等于两个划分(每个划分3个结点)时进入中间优化层
    {
        for(int j=1; j<=transformed_plan.size()-2; j++)
        {
                   pre_pose=transformed_plan[j-1];
                   beh_pose=transformed_plan[j+1];
                   ...
                   if(x_flag&&y_flag)
                   {
                         ...
                         //if(cost<=127)//free space
                         {
                             transformed_plan[j].pose.position.x=mid_pose.pose.position.x;
                             transformed_plan[j].pose.position.y=mid_pose.pose.position.y;
                         }
                           ...
                           std::vector<geometry_msgs::PoseStamped>& plan)
{
    ...
    if (isStartAndGoalCellsValid(startCell, goalCell))
    {
            ...
            //cout<<"improved RRT time="<<(diff(time1,time3).tv_sec)*1e3 + (diff(time1,time3).tv_nsec)*1e-6<<endl;
            //irrtlog<<"improved RRT time="<<(diff(time1,time3).tv_sec)*1e3 + (diff(time1,time3).tv_nsec)*1e-6<<endl;

            //float a=(diff(time1,time2).tv_sec)*1e3 + (diff(time1,time2).tv_nsec)*1e-6;
            //float b=(diff(time1,time3).tv_sec)*1e3 + (diff(time1,time3).tv_nsec)*1e-6;
            //float timeRatio=a/b;

            //cout<<"temperature field time="<<a<<endl;
    ...
    publishPlan(plan);
     ...
     /*
     long t4,t5;
     timespec time4, time5;
     clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time4);
     t4=getCurrentTime();
     for(int i=0;i<500;i++)
     {
         plan=middleoptimizationLayer(plan);
     }
     clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time5);
     t5=getCurrentTime();
     cout<<"optimal time="<<t5-t4<<endl;
     cout<<"optimal time2="<<(diff(time4,time5).tv_sec)*1e3 + (diff(time4,time5).tv_nsec)*1e-6<<endl;
     */
     ...
     cout <<"------The  optimized global path length: "<< path_length<< " meters------"<<endl;
     irrtlog <<"------The  optimized global path length: "<< path_length<< " meters------"<<endl;
     */
...
void  rrt_planner::publishPlan2(const std::vector<geometry_msgs::PoseStamped>& path)
{
    if (!initialized_)
    {
        ROS_ERROR(
...
void rrt_planner::convertToPlan(vector<int>rrtPath,std::vector<geometry_msgs::PoseStamped>& plan, geometry_msgs::PoseStamped  goal,int goalCell)
{
    ...
    */
    ...
    //cout <<"The global path length: "<< path_length<< " meters"<<endl;
    //rrtlog<<"The global path length: "<< path_length<< " meters"<<endl;
}
...
vector <int> rrt_planner::findFreeNeighborCell (int CellID)
{
